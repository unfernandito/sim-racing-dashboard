...
import { FORZA_KEY_MAP } from "@lib/parse-udp-message/types";
...
describe("packetParserV2 suite test", () => {
  let packetParserResult: FORZA_KEY_MAP | undefined;
  ...
  it("should return an error if not receive a valid packet", () => {
    expect(() => packetParser(packetBadValue)).toThrow(Error);
  });

  it("should return an error if not receive a packet", () => {
    expect(() => packetParser(Buffer.from("", "hex"))).toThrow(Error);
  });
});
